...
const { tryStringify } = require("firebase-tools/lib/utils");
...
router.post("/home_loan", async (req, res) => {
  try {
    ...
    // const latestSubmission = await HomeLoanForm.findOne({}, {}, { sort: { 'createdAt': 1 } });
    const latestSubmission = await HomeLoanForm.find().sort({ _id: -1 }).limit(1);
    ...
    if (latestSubmission[0]?.application_no) {
      ...
      nextApplicationNumber = 'PARVHL' + String('0000' + (lastNumber + 1)).slice(-4); // Increment and format
    } else {
      nextApplicationNumber = 'PARVHL0001';
    }
    // }
...
router.post("/home_loan_uploadFiles/:id",
  upload.fields([
    ...
    try {
      ...
      for (const fieldName in req.files) {
        ...
        for (const file of files) {
          ...
          const destinationFileName = `homeloan/${random}${file.originalname}`;
...
router.put("/update_home_Doc/:id",
  ...
  async (req, res) => {
    try {
      ...
      res.status(200).send({ message: "Documents Updated successfully", id: formid });
    } catch (error) {
      console.error("Error adding files to user:", error);
      res.status(500).send("Internal Server Error");
    }
...
router.get("/getallhomeloan/:id", async (req, res) => {
  try {
    // const salariedLoanForm = await SaleriedHomeLoanFormModel.find({ userID: req.params.id });
    // const salariedLoanForm = await HomeLoanForm.find({ userID: req.params.id });
    const HomeLoanById = await HomeLoanForm.findById(req.params.id);
    res.status(200).send({ data: HomeLoanById });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.put("/update_homeLoan/:id", async (req, res) => {
  try {

    const { id } = req.params;
    await HomeLoanForm.findByIdAndUpdate(id, req?.body, { new: true })
...
router.get("/getallhomeloan", async (req, res) => {
  const page = parseInt(req?.query?.page) || 1;
  const limit = parseInt(req?.query?.limit) || 10;
  try {
    // const personalLoanForm = await SaleriedHomeLoanFormModel.find();
    const homeLoanData = await HomeLoanForm.find()
      .sort({ _id: -1 })
      .skip((page - 1) * limit)
      .limit(limit);
    const count = await HomeLoanForm.countDocuments();
    res.status(200).send({
      data: homeLoanData,
...
router.put('/home_loan_status/:id', async (req, res) => {
  const itemId = req.params.id;
  const { status } = req?.body;
  try {

    const updatedItem = await HomeLoanForm.findByIdAndUpdate(itemId, { loan_status: status }, { new: true });

    if (!updatedItem) {
      return res.status(404).json({ message: `Item with ID ${itemId} not found.` });
    }
    const form = await HomeLoanForm.findById(itemId);
...
router.put('/add_homeloan_remark/:id', async (req, res) => {
  const { id } = req?.params;
  const { remark } = req?.body;
  try {
    const addremark = await HomeLoanForm?.findByIdAndUpdate(id, { remarks: remark }, { new: true });
    if (!addremark) {
      return res.status(404).json({ message: `Item with ID ${itemId} not found.` });
    }
...
router.delete("/deletehomeLoan/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const Data = await HomeLoanForm.findById(id);
    Data.files.forEach(async (file) => {
      const filepath = file?.path;
      const deletefromfirebase = await deleteFilefromfirebase(filepath);
      // console.log("deletefromfirebase", deletefromfirebase);
    });
    await HomeLoanForm.findByIdAndDelete(id);
